...
def parse_array(s, _w = WHITESPACE_STR, _sep = ","):
    ...
    for ch in s:
        ...
        if ch == "[":
            ...
            raise ValueError("Wrong string. No started_flag")
        elif ch == "]":
            if not stack:
                raise ValueError("Wrong string. ']'")
            # elif closed_flag:
            ...
            if whitespace_flag and accumulator or closed_flag:
                raise ValueError
            if ch in '+-' and ('+' in accumulator or '-' in accumulator):
                raise ValueError('Two + - signs')

            accumulator += ch
        whitespace_flag = False
    if array is not None and not stack and closed_flag and not sep_flag:
        return array
